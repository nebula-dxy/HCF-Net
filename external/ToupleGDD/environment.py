...
class Environment:
    ...
    def __init__(self, name, graphs, budget, method='RR', use_cache=False, training=True, num_process=5, num_trial=10000):
        '''
            method: 'RR' or 'MC'
            ...
            if self.method == 'MC':
                with Pool(self.num_process) as p:
                    es_inf = statistics.mean(p.map(graph_utils.workerMC, 
                        [[self.graph, S, int(self.num_trial / self.num_process)] for _ in range(self.num_process)]))
            elif self.method == 'RR':
                if self.use_cache:
                    # cached without incremental
                    es_inf = graph_utils.computeRR(self.graph, S, self.num_trial, cache=self.RRs)
                else:
                    es_inf = graph_utils.computeRR(self.graph, S, self.num_trial)
            else:
