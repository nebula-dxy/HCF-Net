...
from scatter_compat import scatter_max
...
class DQAgent:
    ...
    def __init__(self, args):
        ...
        self.init_embed_epochs_train = getattr(args, 'init_embed_epochs_train', 30)
        self.init_embed_epochs_test = getattr(args, 'init_embed_epochs_test', 0)
    ...
    def setup_graph_input_tripling(self, graphs, states, actions=None):
        ...
        for i in range(sample_size):
            ...
            if id(graphs[i]) not in self.graph_node_embed:
                init_epochs = self.init_embed_epochs_train if self.training else self.init_embed_epochs_test
                self.graph_node_embed[id(graphs[i])] = models.get_init_node_embed(graphs[i], init_epochs, self.device)
            with torch.no_grad():
