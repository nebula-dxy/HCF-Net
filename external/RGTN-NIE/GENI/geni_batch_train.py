...
def build_node_dataloader(*args, **kwargs):
    ...
    return dgl.dataloading.DataLoader(*args, **kwargs)
...
def main(args):
    ...
    for cross_id in range(args.cross_num):
        ...
        dataloader = build_node_dataloader(
            g,
            train_idx,
            sampler,
        ...
        for epoch in range(args.epochs):
            ...
            with torch.no_grad():
                ...
                test_loss, test_ndcg, test_spm = rank_evaluate(val_logits[test_idx], labels[test_idx].unsqueeze(-1), 100,
                                                     loss_fcn, spearman=True)
                if not math.isfinite(val_spm):
                    val_spm = 0.0
                if not math.isfinite(test_spm):
                    test_spm = 0.0
...
if __name__ == '__main__':
    ...
    parser.add_argument('--no-scale', dest='scale', action='store_false',
                        help="disable centrality scaling to avoid dead-ReLU collapse on some datasets")
