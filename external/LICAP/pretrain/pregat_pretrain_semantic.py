...
def main(args):
    ...
    for cross_id in range(args.cross_num):
        ...
        feat_pretrained_path = feat_save_root + dataset_name + '_semantic_pregat_pretrained_lr' + str(args.lr) + 'r' + str(args.loss_eta2) + '_' + str(cross_id) + '.pkl'
        ...
        imp_node_coeff = imp_node_coeff.to(device)

        cuda = device.type == 'cuda'
        if cuda:
            g = g.int().to(device)

        num_feats = semantic_feat.shape[1]
        ...
        for epoch in range(args.epochs):
            ...
            if epoch >= 3:
                t0 = time.time()
            # train
            features_pretrained, loss, loss_1, loss_2 = model(semantic_feat, edge_types,  train_idx)
            ...
            # model.eval()
            # with torch.no_grad():
            #     features_pretrained, loss, loss_1, loss_2 = model(semantic_feat, edge_types, val_idx)
        ...
        pk.dump(features_pretrained.detach().cpu(), open(feat_pretrained_path, 'wb'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pretrain-dim', type=int, default=128)
