...
for spec in ACM:acm_nie.pt DBLP:dblp_nie.pt Yelp:yelp_nie.pt; do
  ...
  if [ -f "${PREPARED_DIR}/${dataset}/${file}" ] && [ ! -e "${WORKDIR}/external_prepared/${dataset}/${file}" ]; then
    ln -s "${PREPARED_DIR}/${dataset}/${file}" "${WORKDIR}/external_prepared/${dataset}/${file}"
    echo "[ECS] linked external_prepared/${dataset}/${file}"
  fi
done
...
if [ ! -f "${WORKDIR}/external_prepared/ACM/acm_nie.pt" ] || [ ! -f "${WORKDIR}/external_prepared/DBLP/dblp_nie.pt" ] || [ ! -f "${WORKDIR}/external_prepared/Yelp/yelp_nie.pt" ]; then
  echo "[ECS] external_prepared missing, attempting local generation"
  python prepare_external_nie_data.py | tee "${LOG_DIR}/prepare_external_nie_data.log"
else
  echo "[ECS] using pre-uploaded external_prepared files" | tee "${LOG_DIR}/prepare_external_nie_data.log"
fi
