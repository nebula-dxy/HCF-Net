...
WORKDIR="${WORKDIR:-$(pwd)}"
GPU_ID="${GPU_ID:-0}"
ACM_MAT_PATH="${ACM_MAT_PATH:-${WORKDIR}/data/ACM.mat}"
DBLP_MAT_PATH="${DBLP_MAT_PATH:-${WORKDIR}/data/DBLP.mat}"
YELP_MAT_PATH="${YELP_MAT_PATH:-${WORKDIR}/data/Yelp.mat}"
LOG_DIR="${WORKDIR}/aliyun_interactive_logs"

mkdir -p "${LOG_DIR}"
cd "${WORKDIR}"

echo "[AliyunInteractive] workdir=${WORKDIR}"
echo "[AliyunInteractive] ACM_MAT_PATH=${ACM_MAT_PATH}"
echo "[AliyunInteractive] DBLP_MAT_PATH=${DBLP_MAT_PATH}"
echo "[AliyunInteractive] YELP_MAT_PATH=${YELP_MAT_PATH}"

for path in "${ACM_MAT_PATH}" "${DBLP_MAT_PATH}" "${YELP_MAT_PATH}"; do
  if [ ! -f "${path}" ]; then
    echo "[AliyunInteractive] missing dataset file: ${path}"
    exit 1
  fi
done
...
python credible_sensitivity_runner.py --datasets ACM DBLP Yelp | tee "${LOG_DIR}/credible_sensitivity_runner.log"

echo "[AliyunInteractive] all tasks completed"
